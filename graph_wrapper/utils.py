...
def build_action_map_dict(blue_action_space, actions):
    amap = {a:dict() for a in actions}

    for k,v in blue_action_space.items():
        if k == 'status':
            continue

        # Skip Sleep and Monitor
        if (action_obj := eval(v['name'])) in actions:
            amap[action_obj][v['hostname']] = v['number']

    return amap


def map_action_object(action, target, action_mapping):
    pass
...
def numeric_to_cyborg_action(mapped_action):
    ...
    elif mapped_action["name"] == "Block":
        action = Block(session=session, agent="Blue", hostname=hostname)
